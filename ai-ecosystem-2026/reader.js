(() => {
  ...
  function tick(force = false) {
    ...
    if (force || now-lastFlush >= heartbeat) { if(sessionDelta){add("session_active_time",{visible_ms:Math.min(sessionDelta,30_000)});sessionDelta=0;} for(const [section_id,visible_ms] of sectionDeltas){add("section_active_time",{section_id,visible_ms:Math.min(visible_ms,30_000)});}sectionDeltas.clear();for(const state of blocks.values())state.delta=0;for(const [page_number,visible_ms] of pageDeltas){add("pdf_page_time",{page_number,visible_ms:Math.min(visible_ms,30_000)});}pageDeltas.clear();lastFlush=now;flush(force); }
  }
